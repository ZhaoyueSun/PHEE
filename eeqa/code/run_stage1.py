...
def read_query_templates(normal_file):
    ...
    with open(normal_file, "r", encoding='utf-8') as f:
        for line in f:
            ...
            # 2 template arg_name + in trigger (replace [trigger] when forming the instance)
                    ...
                    if (step + 1) % eval_step == 0 or step == 0:
                        ...
                        if args.do_eval:
                            ...
                            if (best_result is None) or (result[args.eval_metric] > best_result[args.eval_metric]):
                                ...
                                logger.info("!!! Best dev %s (lr=%s, epoch=%d): CLS_EM_F1: %.2f, CLS_Micro_F1: %.2f" %
                                            (args.eval_metric, str(lr), epoch, result["CLS_Arguments_EM_F1"], result["CLS_Arguments_MICRO_F1"]))
                ...
                for lid, line in enumerate(f.readlines()):
                    ...
                    for event in dat['event']:
                        if event:
                            ...
                            events[ev_type]['Trigger'].append([trigger_ofs, trigger_ofs, ev_type])
                    pred_rst = preds[lid]
                    for p in pred_rst:
