...
def evaluate(args, eval_examples, category_vocab, model, device, eval_dataloader, pred_only=False):
    ...
    for sentence_id, _ in enumerate(pred_events):
        ...
        for trigger in gold_sent_events:
            gold_trigger_n += 1
        # for true_positive_n
        common = collections.Counter(gold_sent_events) & collections.Counter(pred_sent_events)
    ...
    result = collections.OrderedDict([('prec_c',  prec_c), ('recall_c',  recall_c), ('f1_c', f1_c),
     ('prec_i',  prec_i), ('recall_i',  recall_i), ('f1_i', f1_i),
     ('prec_e',  prec_e), ('recall_e',  recall_e), ('f1_e', f1_e)])
...
def main(args):
                    ...
                    if (step + 1) % eval_step == 0 or step == 0:
                        ...
                        if args.do_eval:
                            ...
                            if (best_result is None) or (result[args.eval_metric] > best_result[args.eval_metric]):
                                ...
                                logger.info('Epoch: {}, Step: {} / {}, used_time = {:.2f}s, loss = {:.6f}'.format(
                                    epoch, step + 1, len(train_batches), time.time() - start_time, tr_loss / nb_tr_steps))
                                logger.info("!!! Best dev %s (lr=%s, epoch=%d): p_c: %.2f, r_c: %.2f, f1_c: %.2f, p_i: %.2f, r_i: %.2f, f1_i: %.2f, p_e: %.2f, r_e: %.2f, f1_e: %.2f" %
                                            (args.eval_metric, str(lr), epoch, result["prec_c"], result["recall_c"], result["f1_c"], result["prec_i"], result["recall_i"], result["f1_i"],
                                            result["prec_e"], result["recall_e"], result["f1_e"]))
                        else:
                            save_model = True
    ...
    if args.do_eval:
        ...
        if args.add_lstm:
            model = BertLSTMForTriggerClassification.from_pretrained(model_dir, num_labels=len(category_vocab.index_to_category))
        else:
            model = BertForTriggerClassification.from_pretrained(model_dir, num_labels=len(category_vocab.index_to_category))
        if args.fp16:
            model.half()
        model.to(device)
