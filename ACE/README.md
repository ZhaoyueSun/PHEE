...
The code is borrowed from the follow paper and fitted as one of our baselines:
[Automated Concatenation of Embeddings for Structured Prediction](https://arxiv.org/abs/2010.05006)

Original Code Link: [Github](https://github.com/Alibaba-NLP/ACE)

# Instructions:
preprocess json data to ace format:
```
...
fintune bert embedding:
```
conda activate ace
cd ACE
CUDA_VISIBLE_DEVICES=0 python train.py --config config/bert-phee-finetune.yaml
```

finetune biobert embedding:
```
CUDA_VISIBLE_DEVICES=0 python train.py --config config/biobert-phee-finetune.yaml
```

train ACE:
```
CUDA_VISIBLE_DEVICES=0 python train.py --config config/phee_ace.yaml
```

evaluate ACE model result:
```
cd preprocess
python eval_ace_pred.py
```


# ACE README:

The code is for our ACL-IJCNLP 2021 paper: 
